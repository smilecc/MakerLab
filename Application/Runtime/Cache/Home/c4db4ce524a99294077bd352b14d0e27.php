...
<title>填写活动申请表单 - <?php echo C('SITE_TITLE');?></title>

<form class="am-form" method="post" action="<?php echo U('/Home/Activity/SetForm');?>">
...
<li><span class="am-badge am-radius am-text-sm">等待审核</span></li>
  ...
  <fieldset>
    ...
    <legend>填写活动申请表单 <small><?php echo ($proInfo['name']); ?></small></legend>

    <?php if(is_array($questionList)): $i = 0; $__LIST__ = $questionList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="am-form-group">
        <label for="doc-ipt-email-1"><?php echo ($vo['question']); ?></label>
      <?php if($vo['input_element'] == 0): ?><input type="text" name="<?php echo ($vo['id']); ?>" name="name" required/>
      <?php else: ?>
          <textarea name="<?php echo ($vo['id']); ?>" rows="6"required/></textarea><?php endif; ?>
      </div><?php endforeach; endif; else: echo "" ;endif; ?>
    <p><button type="submit" class="am-btn am-btn-success">提交申请</button></p>
  </fieldset>
</form>
