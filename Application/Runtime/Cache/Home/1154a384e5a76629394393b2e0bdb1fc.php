...
<title>修改项目 - <?php echo C('SITE_TITLE');?></title>
...
<form class="am-form" method="post" action="<?php echo U('/Home/Project/addprj');?>">
  <fieldset>
    ...
    <div class="am-form-group">
      ...
      <input type="text" value="<?php echo ($proinfo['name']); ?>" class="" id="doc-ipt-email-1" name="name" placeholder="你的项目叫什么呢？" required/>
    </div>
    ...
    <div class="am-form-group am-form-file">
      ...
      <div>
        <button type="button" class="am-btn am-btn-default am-btn-sm">
          ...
          <img class="am-radius am-img-thumbnail" id="imgdis" src="/Public/Uploads/<?php echo ($proinfo['img']); ?>" alt=""/>
      </div>
...
<ul class="am-avg-sm-3 am-thumbnails">
  <li>    
  	<div class="am-form-group">
		...
		<?php if(is_array($process)): $i = 0; $__LIST__ = $process;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo['id']); ?>" <?php if($proinfo['process'] == $vo['id']): ?>selected<?php endif; ?>><?php echo ($vo['name']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
		</select>
		</div>
    </div>
  </li>
  <li>
      <div class="am-form-group">
		...
		<?php if(is_array($type)): $i = 0; $__LIST__ = $type;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo['id']); ?>" <?php if($proinfo['type'] == $vo['id']): ?>selected<?php endif; ?>><?php echo ($vo['name']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
		</select>
		</div>
    </div>
  </li>
    <li>
      <div class="am-form-group">
		...
		<?php if(is_array($tag)): $i = 0; $__LIST__ = $tag;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo['id']); ?>" <?php if($proinfo['tag'] == $vo['id']): ?>selected<?php endif; ?>><?php echo ($vo['name']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
		</select>
		</div>
    </div>
  </li>
    ...
    <div class="am-form-group">
      ...
      <textarea name="intro" class="" rows="6" id="doc-ta-1" required/><?php echo ($proinfo['intro']); ?></textarea>
    </div>
    <input type="hidden" name="id" value="<?php echo ($proinfo['id']); ?>" />
    <p><button type="submit" class="am-btn am-btn-success">提交</button>
    <a class="am-btn am-btn-danger" href="<?php echo U('/Home/Project/detail').'?id='.$proinfo['id'];?>">返回项目页面</a>
    </p>
  </fieldset>
</form>
